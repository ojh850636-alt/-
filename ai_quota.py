    ...
    with _lock:
        ...
        try:
            dbg = {
                ...
                "usage_path": path,
                "AI_DAILY_CALL_LIMIT": os.getenv("AI_DAILY_CALL_LIMIT"),
                "AI_MAX_CALLS_PER_MINUTE": os.getenv("AI_MAX_CALLS_PER_MINUTE"),
            }
...
def call_provider(payload: dict):
    ...
    try:
        if (
            ...
            and payload.get("mock_error")
            and os.getenv("AI_USE_MOCK", "false").lower() in ("1", "true", "yes")
        ):
        ...
        with open(WEBHOOK_LOG, "a", encoding="utf-8") as f:
            f.write(
                json.dumps(
                    {
                        "time": datetime.now(timezone.utc).isoformat(),
                        "action": "ai_quota_delegating_to_providers",
