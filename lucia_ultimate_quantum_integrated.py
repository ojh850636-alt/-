...
GROQ_API_URL = os.getenv(
    "GROQ_API_URL"
)  # full endpoint, e.g. https://api.groq.ai/v1/...
...
OPENROUTER_AVAILABLE = bool(
    OPENROUTER_API_KEY and OPENROUTER_URL and requests is not None
)
...
async def index(request: Request):
    ...
    if tpl.exists():
        return templates.TemplateResponse(
            "index.html", {"request": request, "ai_available": ai_available}
        )
...
async def ai_chat(payload: Dict[str, Any]):
    ...
    if ai_available and ai_client is not None:
        try:
            ...
            def call_openai():
                return ai_client.ChatCompletion.create(
                    model=model,
            ...
            try:
                resp = await asyncio.wait_for(
                    asyncio.to_thread(call_openai), timeout=ai_timeout
                )
            ...
            return {
                "ok": False,
                "message": "ai call failed, check server logs for details",
            }
        ...
        try:
            ok, res = await asyncio.wait_for(
                _call_openrouter_async(
                    text, timeout=float(os.getenv("AI_TIMEOUT", "10"))
                ),
    ...
    if GROQ_AVAILABLE:
        try:
            ok, res = await asyncio.wait_for(
                _call_groq_async(text, timeout=float(os.getenv("AI_TIMEOUT", "10"))),
                timeout=float(os.getenv("AI_TIMEOUT", "10")),
            )
            if ok:
                return {"ok": True, "response": res}
