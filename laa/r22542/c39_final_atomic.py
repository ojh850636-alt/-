...
def target_names(ctl):
 b=max(1,ctl.nl//3);return sorted(x['name'] for x in ctl.mods if x['layer']<b and x['proj'] in {'gate_proj','up_proj','down_proj'})
def apply_target(ctl,condition):
 names=target_names(ctl);ns=set(names)
 if condition=='TARGET_ABLATION':
  ctl.all(1.0)
  for x in ctl.mods:
...
def compact_score(model,tok,rows,bs=8):
 ...
 return out
def generate(model,tok,rows,condition,h,seed=42,k=8,prompt_batch=4):
 import torch
...
def load(base,adapter):
 ...
 h=loadmod('laa/r22542/c39_behavior.py','c39_behavior_atomic_final')
...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--mode',choices=['semantic','generation'],required=True);ap.add_argument('--condition',choices=['TARGET_ONLY','TARGET_ABLATION'],required=True);ap.add_argument('--split',choices=['CONFIRMATION','GENERATOR_HOLDOUT','ALPHA_RENAME_OOD','NEGATIVE_UNPROVABLE'],required=True);ap.add_argument('--base',required=True);ap.add_argument('--adapter',required=True);ap.add_argument('--out',required=True);ap.add_argument('--private');a=ap.parse_args()
 m,tok,ctl,h,names=load(a.base,a.adapter);apply_target(ctl,a.condition);site_sha=hashlib.sha256('\n'.join(names).encode()).hexdigest();suite=h.build_suite()
 if a.mode=='semantic':
  assert a.split in {'CONFIRMATION','GENERATOR_HOLDOUT'};rows=[r for r in suite if r['positive'] and r['split']==a.split];assert len(rows)==64
  res=compact_score(m,tok,rows);dump(a.out,{'schema':'LUCIA_AA_R22542_C39_FINAL_ATOMIC_SEMANTIC_V1','target':'EARLY_BAND&MLP_ALL','site_count':24,'site_names_sha256':site_sha,'condition':a.condition,'split':a.split,'case_count':64,'case_results':res,'fresh_process':True,'raw_weights_saved':False,'raw_logits_saved':False,'raw_activations_saved':False,'hf_model_redownloads':0})
 else:
  assert a.private;rows=sorted([r for r in suite if r['split']==a.split],key=lambda x:x['case_id'])[:12];assert len(rows)==12
  rec=generate(m,tok,rows,a.condition,h);Path(a.private).write_text(''.join(json.dumps(x,sort_keys=True,ensure_ascii=False)+'\n' for x in rec),encoding='utf-8');dump(a.out,{'schema':'LUCIA_AA_R22542_C39_FINAL_ATOMIC_GENERATION_RECEIPT_V1','target':'EARLY_BAND&MLP_ALL','site_count':24,'site_names_sha256':site_sha,'condition':a.condition,'split':a.split,'seed':42,'case_count':12,'records':len(rec),'raw_tactics_exported_public':False,'hf_model_redownloads':0})
