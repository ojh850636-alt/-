    ...
    with torch.inference_mode():
        for st in range(0,len(rows),bs):
            ...
            for i,(r,nc) in enumerate(zip(rr,cl)):
                v=lp[i,-nc-1:-1,:].gather(-1,ii[i,-nc:].unsqueeze(-1)).squeeze(-1)
                out.append({'case_id':r['case_id'],'split':r['split'],'family':r['family'],'mean_logprob':float(v.mean().cpu()),'token_count':nc})
    return out

def generate(model,tok,rows,condition,seed=42,k=8,prompt_batch=4):
    h=loadmod('final/c39_behavior_pinned.py','c39_behavior_gen_helper')
    ...
    with torch.inference_mode():
        for st in range(0,len(rows),prompt_batch):
            rr=rows[st:st+prompt_batch]
            e=tok([r['prompt'] for r in rr],return_tensors='pt',padding=True,add_special_tokens=False,truncation=True,max_length=768);e={a:b.to(model.device) for a,b in e.items()};n=e['input_ids'].shape[1]
            ...
            for j,r in enumerate(rr):
                for rank in range(k):
                    raw=tok.decode(g[j*k+rank,n:],skip_special_tokens=True);t,stt=h.sanitize(raw)
                    out.append({'condition':condition,'seed':seed,'case_id':r['case_id'],'split':r['split'],'family':r['family'],'rank':rank,'status':stt,'tactic':t,'tactic_sha256':hashlib.sha256(t.encode()).hexdigest() if t else None})
    return out
...
def load():
    ...
    h=loadmod('final/c39_behavior_pinned.py','c39_behavior_final_helper')
...
def semantic():
    target,h,tok,model,ctl,tg=load();rows=[r for r in h.build_suite() if r['positive'] and r['split'] in {'CONFIRMATION','GENERATOR_HOLDOUT'}];assert len(rows)==128
    ctl.all(1);full=score(model,tok,rows)
    with model.disable_adapter():base=score(model,tok,rows)
    ctl.all(1)
    ns=set(tg)
    for x in ctl.mods:
        if x['name'] in ns:x['m'].scaling[x['key']]=0.0
    ablation=score(model,tok,rows)
    ctl.all(0)
    for x in ctl.mods:
        if x['name'] in ns:x['m'].scaling[x['key']]=x['scale']
    only=score(model,tok,rows)
...
def generation():
    mode=os.environ['C39_GEN_MODE'];assert mode in {'TARGET_ONLY','TARGET_ABLATION'}
    target,h,tok,model,ctl,tg=load();rows=h.build_suite();locked=[]
    ...
    if mode=='TARGET_ONLY':
        ...
        for x in ctl.mods:
            if x['name'] in ns:x['m'].scaling[x['key']]=0.0
    rec=generate(model,tok,locked,mode,42,8,4);Path('private').mkdir(exist_ok=True);Path('private/TACTICS.jsonl').write_text(''.join(json.dumps(x,sort_keys=True,ensure_ascii=False)+'\n' for x in rec),encoding='utf-8')
...
if __name__=='__main__':
    m=os.environ['C39_FINAL_MODE']
    semantic() if m=='semantic' else generation()
