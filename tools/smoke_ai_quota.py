...
def run():
    ...
    try:
        ...
        results['first'] = {'status': r.status_code, 'body': r.json() if r.headers.get('content-type','').startswith('application/json') else r.text}

        r2 = client.post('/ai/chat', json={'prompt':'ok','mock': True})
        results['second'] = {'status': r2.status_code, 'body': r2.json() if r2.headers.get('content-type','').startswith('application/json') else r2.text}
