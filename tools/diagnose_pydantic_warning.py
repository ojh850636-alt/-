...
with out.open('w', encoding='utf-8') as f:
    ...
    for name in ['lucia_ultimate_quantum_integrated_fixed', 'lucia_ultimate_quantum_integrated']:
        f.write('\nMODULE: %s\n' % name)
        try:
            ...
            try:
                ...
                occurrences = [ (i+1, l) for i,l in enumerate(lines) if '.dict(' in l or 'req.dict' in l ]
                f.write('\nOCCURRENCES OF .dict(:\n')
    ...
    for k in sorted(sys.modules.keys()):
        if 'lucia_ultimate' in k:
            f.write(k + ' -> ' + str(getattr(sys.modules[k], '__file__', 'built-in or none')) + '\n')
print('WROTE', out)
