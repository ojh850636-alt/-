...
def main():
    ...
    report = {'time': datetime.now(timezone.utc).isoformat(), 'pytest': None, 'smoke_ai_quota': None, 'run_concurrency_smoke': None}
    ...
    try:
        report['smoke_ai_quota'] = run_smoke('tools.smoke_ai_quota', 'smoke_ai_quota', repo_root)
    except Exception as e:
        report['smoke_ai_quota'] = {'ok': False, 'error': repr(e)}

    try:
        report['run_concurrency_smoke'] = run_smoke('tools.run_concurrency_smoke', 'run_concurrency_smoke', repo_root)
    except Exception as e:
