...
with open(
    os.path.join(os.path.dirname(__file__), "ai_quota_smoketest_out.json"),
    "w",
    encoding="utf-8",
) as f:
    f.write(
        json.dumps({"r1_status": r1.status_code, "r1": r1.json()}, ensure_ascii=False)
    )
...
with open(
    ...
    f.write("\n")
    f.write(
        json.dumps({"r2_status": r2.status_code, "r2": r2.json()}, ensure_ascii=False)
    )
print("wrote")
