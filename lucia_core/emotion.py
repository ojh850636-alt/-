...
class EmotionSystem:
    ...
    def update(
        self, threat_level: int, external_input: str = None
        ...
        self.emotion_vector = np.clip(
            self.emotion_vector + emotion_weights - self.decay_rates, 0, 1
        )
        self.emotion_vector[0] = max(0, 1 - sum(self.emotion_vector[1:]))
        dominant_emotion = ["평온", "기쁨", "호기심", "경계", "불안", "분노", "공포"][
            int(np.argmax(self.emotion_vector))
        ]
