...
def _call_openai(payload: dict):
    ...
    try:
        ...
        # Real call would go here; we return a simulated response for now
...
def call_best_provider(payload: dict):
    ...
    try:
        ...
        with open(logp, 'a', encoding='utf-8') as lf:
            lf.write(json.dumps({'time': datetime.now(timezone.utc).isoformat(), 'action': 'provider_received_payload', 'module_file': __file__, 'pid': os.getpid(), 'keys': list(payload.keys()) if isinstance(payload, dict) else None, 'mock_error': bool(payload.get('mock_error')) if isinstance(payload, dict) else None}, ensure_ascii=False) + '\n')
    except Exception:
        pass
    ...
    try:
        if isinstance(payload, dict) and (payload.get('prompt') == 'will error' or payload.get('text') == 'will error'):
            with open(logp, 'a', encoding='utf-8') as lf:
                lf.write(json.dumps({'time': datetime.now(timezone.utc).isoformat(), 'action': 'provider_forced_error', 'module_file': __file__, 'pid': os.getpid(), 'prompt': payload.get('prompt')}, ensure_ascii=False) + '\n')
    ...
    for p in order:
        p = p.strip().upper()
        if p == 'OPENAI' and os.getenv('ENABLE_OPENAI', 'true').lower() in ('1','true','yes'):
            try:
                return _call_openai(payload)
            except Exception:
                continue
        if p == 'GROQ' and os.getenv('ENABLE_GROQ', 'false').lower() in ('1','true','yes'):
            try:
                return _call_groq(payload)
            except Exception:
                continue
        if p == 'OPENROUTER' and os.getenv('ENABLE_OPENROUTER', 'false').lower() in ('1','true','yes'):
            try:
